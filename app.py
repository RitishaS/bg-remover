...
def process_frame(frame):
    ...
    if background_image is not None:
        ...
        removed_bg = remove(frame_rgba, session=session)
        
        # Prepare background
        bg_resized = cv2.cvtColor(
            cv2.resize(background_image, (frame.shape[1], frame.shape[0])),
            cv2.COLOR_BGR2RGB
        )
        ...
        foreground = cv2.cvtColor(np.array(removed_bg), cv2.COLOR_RGBA2RGB)
...
# Add this new endpoint for image background replacement
...
def stop_recording():
    ...
    if recording:
        ...
        if video_writer is not None:
            ...
            if recordings:
                finalize_video(os.path.join(RECORDINGS_FOLDER, recordings[0]))
